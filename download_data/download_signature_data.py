...
OUTPUT_DIR = "data_signature_only"
...
def run():
    ...
    loc2 = download_project(PROJ2_WORKSPACE, PROJ2_NAME, PROJ2_VERSION, "data_signature_only/raw_proj2")
    
    # Extract based on new logic
    data2 = get_data_from_coco(loc2, negatives_only=False) # Mixed docs from Proj 2
    
    # Stratify
    pos = [s for s in data2 if len(s["anns"]) > 0]
    neg = [s for s in data2 if len(s["anns"]) == 0]
    ...
    print("Done! Dataset signature only ready for Qwen training.")
