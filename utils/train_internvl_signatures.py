...
# Expected local YOLO dataset structure:
# data/Signature-Detection-1/
#   train/images
#   train/labels
#   valid/images
#   valid/labels
#   test/images
#   test/labels
DATA_ROOT = Path("data/Signature-Detection-1")

# Converted InternVL JSONL output
OUT_DIR = Path("data/internvl_signature_data")

# Pretrained adapter zip stored in repo
PRETRAINED_ADAPTER_ZIP = Path("weights/internvl_vision_LORA_signature_ft.zip")

# Temporary unzip directory
PRETRAINED_ADAPTER_UNZIP_DIR = Path("weights/internvl_vision_LORA_signature_ft")

# Output directory for continued fine-tuning result
OUTPUT_DIR = Path("outputs/internvl_signature_lora_ft/final")
...
def find_adapter_dir(root: Path) -> Path:
    ...
    raise FileNotFoundError(f"Could not find a valid adapter directory inside: {root}")
...
def unzip_adapter(zip_path: Path, extract_dir: Path) -> Path:
    ...
    return find_adapter_dir(extract_dir)
